...
- Hero mark: `mark-compass`, a lime quatrefoil (four-arc geometric motif) that draws in as a live construction diagram — axes, outer circle, square, dashed diagonals, inner circle, dashed diamond, then the final outline and fill — using olive-toned guide lines (`var(--olive)`/`var(--olive-line)`), the same `.draw-line`/`.fade-up` reveal classes as the rest of the illustration system. The final shape's fill is a *separate* element from its outline stroke, faded in only once the outline finishes drawing — SVG `fill` ignores `stroke-dashoffset`, so a combined fill+stroke path would render its fill fully opaque from frame one and silently hide any guide line inside its silhouette. Viewbox is 0–1000 (not the 0–240 the rest of the sprite uses) with correspondingly heavier stroke widths, since it's the only illustration rendered this large.
