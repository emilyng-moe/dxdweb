...
- Hero mark: `mark-compass`, a quatrefoil (four-arc geometric motif) that draws in as a live construction diagram — axes, outer circle, square, dashed diagonals (revealed via a moving mask, not a plain fade), inner circle, dashed diamond, then a stroke-only final outline (no fill) and four corner nodes. Ported faithfully from a reference artifact: its own colors (`#7a8c55`/`#a3ab8e`/`#8e9a70`/`#bce462`, not DXD's tokens), stroke weights, and exact timing/easing (`.mc-*` classes in base.css), not adapted to the site's usual palette. Two deliberate exceptions: no background grid tile (the hero already sits inside `.blueprint`'s own grid) and it plays once via the site's `--illus-play` scroll-trigger gate instead of the original's own replay-button/loop demo chrome, which has no equivalent on a page a visitor scrolls past. Viewbox is 0–1000 (not the 0–240 the rest of the sprite uses), so at this hero's ~370px display size the thinnest guide lines (2px in source) render quite subtly — that's the source material's own weights, kept as designed rather than bumped up.
